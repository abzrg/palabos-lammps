/*
 * This file is part of the PalabosLammps software.
 *
 * PalabosLammps is free software: you can redistribute it and/or modify
 ...
 *
 * Author: Jifu Tan (tanjifu@gmail.com; jifutan@niu.edu)
 */
...
public:
    Interpolation3D(LammpsWrapper &wrapper_) : wrapper(wrapper_)
    {
        ...
        for (i = 0; i < nfix; i++)
            if (strcmp(wrapper.lmp->modify->fix[i]->style, "fcm") == 0)
                ifix = i;
    ...
    virtual void process(Box3D domain, BlockLattice3D<T, Descriptor> &lattice)
    {
        ...
        // std::cout<<"velocity size "<<lattice.getNx()<<" "<<lattice.getNy()<<"
        // "<<lattice.getNz()<<std::endl;
                    ...
                    for (jj = 0; jj < 4; jj++)
                        for (kk = 0; kk < 4; kk++) {
                            ...
                            /*     if (ix > domain.x1 + 2 || ix < domain.x0 - 2)
                                     std::cout<<" out x "<<domain.x0<<" "<<domain.x1<<" x
                               "<<x[iS][0]<<" nlocal, id "<<nlocal<<" "<<iS<<" ix "<<ix
                               <<" offset "<<offset.x<<" "<<offset.y<<" "<<offset.z<<"
                               "<<std::endl; if (iy > domain.y1 + 2 || iy < domain.y0 - 2)
                                     std::cout<<" out y "<<domain.y0<<" "<<domain.y1<<" y
                               "<<x[iS][1]<<" nlocal, id "<<nlocal<<" "<<iS<<" iy "<<iy
                               <<" offset "<<offset.x<<" "<<offset.y<<" "<<offset.z<<"
                               "<<std::endl; if (iz > domain.z1 + 2 || iz < domain.z0 - 2)
                                     std::cout<<" out z "<<domain.z0<<" "<<domain.z1<<" z
                               "<<x[iS][2]<<" nlocal, id "<<nlocal<<" "<<iS<<" iz "<<iz
                               <<" offset "<<offset.x<<" "<<offset.y<<" "<<offset.z<<"
                               "<<"zl "<<zl<<" "<<" kk "<<kk<<std::endl;*/
...
void interpolateVelocity3D(MultiBlockLattice3D<T, Descriptor> &lattice, LammpsWrapper &wrapper)
{
    ...
    // applyProcessingFunctional(new Interpolation3D<T>(wrapper),
    // velocity.getBoundingBox(),velocity, envelopeWidth);
    applyProcessingFunctional(
        new Interpolation3D<T, Descriptor>(wrapper), lattice.getBoundingBox(), lattice);
}
...
public:
    ...
    virtual void process(Box3D domain, TensorField3D<T, 3> &velocity)
    {
        ...
        // TensorField3D<T,Descriptor<T>::d>
        // velocity(lattice.getNx(),lattice.getNy(),lattice.getNz());
        // computeVelocity(lattice, velocity);
        // std::cout<<"velocity size "<<lattice.getNx()<<" "<<lattice.getNy()<<"
        // "<<lattice.getNz()<<std::endl;
        plint xl, yl, zl, ix, iy, iz, ii, jj, kk;
        ...
        //      if (iy > domain.y1 || iy < domain.y0) continue;
          //    if (iz > domain.z1 || iz < domain.z0) continue;
             // vicinity = 1 can access x0-1 or x1+1
              if (ix > domain.x1 + 2 || ix < domain.x0 - 2)
                  std::cout<<" out x "<<domain.x0<<" "<<domain.x1<<" x
    "<<x[iS][0]<<" nlocal, id "<<nlocal<<" "<<iS<<" ix "<<ix <<" offset
    "<<offset.x<<" "<<offset.y<<"
    "<<offset.z<<" "<<std::endl; if (iy > domain.y1 + 2 || iy < domain.y0 - 2)
                  std::cout<<" out y "<<domain.y0<<" "<<domain.y1<<" y
    "<<x[iS][1]<<" nlocal, id "<<nlocal<<" "<<iS<<" iy "<<iy <<" offset
    "<<offset.x<<" "<<offset.y<<"
    "<<offset.z<<" "<<std::endl; if (iz > domain.z1 + 2 || iz < domain.z0 - 2)
                  std::cout<<" out z "<<domain.z0<<" "<<domain.z1<<" z
    "<<x[iS][2]<<" nlocal, id "<<nlocal<<" "<<iS<<" iz "<<iz <<" offset
    "<<offset.x<<" "<<offset.y<<"
    "<<offset.z<<" "<<"zl "<<zl<<" "<<" kk "<<kk<<std::endl; uf =
    velocity.get(ix,iy,iz); wgt = wx[ii]*wy[jj]*wz[kk]; us[0] += wgt*uf[0]; us[1]
    += wgt*uf[1]; us[2] += wgt*uf[2];
            }
        v[iS][0]=us[0];
        v[iS][1]=us[1];
        v[iS][2]=us[2];
...
void interpolateVelocity3D(MultiTensorField3D<T, 3> &velocity, LammpsWrapper &wrapper)
{
    ...
    // velocity.getBoundingBox(),velocity, envelopeWidth);
    applyProcessingFunctional(
        new Interpolation3D_T<T>(wrapper), velocity.getBoundingBox(), velocity);
}
                ...
                for (ii = 0; ii < 4; ii++)
                    for (jj = 0; jj < 4; jj++)
                        for (kk = 0; kk < 4; kk++) {
                            ...
                            // uf = velocity.get(ix,iy,iz);
                            /*if (ix > domain.x1 + 2  || ix < domain.x0 - 2)
                                std::cout<<" spread out x "<<domain.x0<<" "<<domain.x1<<"
                            x "<<x[iS][0]<<" nlocal, id "<<nlocal<<" "<<iS<<" ix "<<ix <<"
                            offset "<<offset.x<<" "<<offset.y<<" "<<offset.z<<"
                            "<<std::endl; if (iy > domain.y1 + 2 || iy < domain.y0 - 2)
                                std::cout<<" spread out y "<<domain.y0<<" "<<domain.y1<<"
                            y "<<x[iS][1]<<" nlocal, id "<<nlocal<<" "<<iS<<" iy "<<iy <<"
                            offset "<<offset.x<<" "<<offset.y<<" "<<offset.z<<"
                            "<<std::endl; if (iz > domain.z1 + 2 || iz < domain.z0 - 2)
                                std::cout<<" spread out z "<<domain.z0<<" "<<domain.z1<<"
                            z "<<x[iS][2]<<" nlocal, id "<<nlocal<<" "<<iS<<" iz "<<iz <<"
                            offset "<<offset.x<<" "<<offset.y<<" "<<offset.z<<" "<<"zl
                            "<<zl<<" "<<" kk "<<kk<<std::endl;*/
                            ...
                            cell.setExternalField(
                                ...
                                Descriptor<T>::ExternalField::sizeOfForce, ff);
                        }
            }  // mask[iS]
        }
        /*
        pcout<<"lbm x0 "<<domain.x0<<" "<<domain.x1<<" nx
        "<<lattice.getNx()<<std::endl; pcout<<"lbm y0 "<<domain.y0<<"
        "<<domain.y1<<" ny "<<lattice.getNy()<<std::endl; pcout<<"lbm z0
        "<<domain.z0<<" "<<domain.z1<<" nz "<<lattice.getNz()<<std::endl; for
        (ix=domain.x0;ix<=domain.x1;ix++) for (iy=domain.y0;iy<=domain.y1;iy++) for
        (iz=domain.z0;iz<=domain.z1;iz++){ Cell<T,Descriptor> &cell =
        lattice.get(ix+2,iy+2,iz+2); Cell<T,Descriptor> &cell2 =
        lattice.get(ix-2,iy-2,iz-2); }*/
    }
...
void spreadForce3D(MultiBlockLattice3D<T, Descriptor> &lattice, LammpsWrapper &wrapper)
{
    // plint envelopeWidth = 2;
    applyProcessingFunctional(
        new Spreading3D<T, Descriptor>(wrapper), lattice.getBoundingBox(), lattice);
}
...
public:
    ...
    virtual void process(Box3D domain, BlockLattice3D<T, Descriptor> &lattice)
    {
        ...
        TensorField3D<T, Descriptor<T>::d> velocity(
            lattice.getNx(), lattice.getNy(), lattice.getNz());
        // ScalarField3D<T>
        // density(lattice.getNx(),lattice.getNy(),lattice.getNz());
        // computeVelocity(lattice, velocity);
        plint xl, yl, zl, ix, iy, iz, ii, jj, kk;
        T rx, ry, rz, wgt;
        T rho;
        ...
        for (plint iS = 0; iS < nlocal; iS++) {
            if (mask[iS] & groupbit) {
                ...
                // assume rho = 1.
                // fsi[0]=us[0]-v[iS][0];       //Eqn.(5) in K. Aidun. Int J. Numer.
                // Meth. Fluids 2010:62:765-783 fsi[1]=us[1]-v[iS][1];
                // fsi[2]=us[2]-v[iS][2]; using rho value
                fsi[0] = rho * (us[0] - v[iS][0]);  // Eqn.(5) in K. Aidun. Int J. Numer.
                                                    // Meth. Fluids 2010:62:765-783
                ...
                fe[iS][1] = fsi[1];
                fe[iS][2] = fsi[2];
                // std::cout<<"force on particle "<<fsi[0]<<" "<<fsi[1]<<"
                // "<<fsi[2]<<std::endl;
...
void forceCoupling3D(MultiBlockLattice3D<T, Descriptor> &lattice, LammpsWrapper &wrapper)
{
    ...
    // velocity.getBoundingBox(),velocity, envelopeWidth);
    applyProcessingFunctional(
        new ForceFSI3D<T, Descriptor>(wrapper), lattice.getBoundingBox(), lattice);
}
