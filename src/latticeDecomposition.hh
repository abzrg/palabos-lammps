...
LatticeDecomposition::LatticeDecomposition(
    plb::plint nx_, plb::plint ny_, plb::plint nz_, LAMMPS_NS::LAMMPS *lmp_) :
    nx(nx_), ny(ny_), nz(nz_), lmp(*lmp_), blockStructure(0), threadAttribution(0)
{
    ...
    for (plint i = 0; i <= npz; i++) {
        ...
        // pcout<<"lammps domain "<<lmp.domain->zprd<<" lo-hi in z "<<lmp.domain->sublo[2]<<"
        // "<<lmp.domain->subhi[2]<<std::endl;
    }

    // pcout<<"domain lb "<<xVal[0]<<" "<<xVal.back()<<" y "<<yVal[0]<<" "<<yVal.back()<<" z
    // "<<zVal[0]<<" "<<zVal.back()<<std::endl;

    blockStructure = new SparseBlockStructure3D(
        ...
        for (plint iY = 0; iY < yVal.size() - 1; ++iY) {
            for (plint iZ = 0; iZ < zVal.size() - 1; ++iZ) {
                ...
                blockStructure->addBlock(
                    Box3D(
                        xVal[iX], xVal[iX + 1] - 1, yVal[iY], yVal[iY + 1] - 1, zVal[iZ],
                        zVal[iZ + 1] - 1),
                    id);
